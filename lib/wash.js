...
function deepDefaults(dest, src) {
...
function Precompiled(code, options) {
    ...
    deepDefaults(self.options, defaultOptions);
}
...
Precompiled.prototype.render = function(context) {
    ...
    try {
        ...
        if(self.options.throwsOnErrors) { throw e; }
        else { return ''; }
    }
...
Wash.prototype.precompile = function() {
    ...
    _.each(tokens, function(token) {
        ...
        try {
            ...
            if((match = evalTagRegex.exec(token)) && (tagLevels.raw <= 0)) {
                ...
                if(expr) {
                    ...
                    if(tagLevels.raw <= 0) {
                        ...
                        self._code += '__out+="' + escapeStr(token) + '";\n';
                    }
            ...
            if(self.options.throwsOnErrors) { throw err; }
        }
    });

    _.each(tagLevels, function(level, tagName) {
        if(level > 0) {
            console.assert(!self.options.throwsOnErrors, 'No closing tag: ' + tagName);
            self._code = 'var __out = ""';
        } else if(level < 0) {
            console.assert(!self.options.throwsOnErrors, 'Redundant closing tag: ' + tagName);
            self._code = 'var __out = ""';
        }
    });
