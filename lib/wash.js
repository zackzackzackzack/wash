...
var actionTagNameRegex = new RegExp(escapeRegex(actionOpenTag) + '\\s*(\\w+)[^]*' + escapeRegex(actionCloseTag));
...
Wash.prototype._check = function(tokens) {
    var levels = {
        'for': 0,
        'if': 0,
        'raw': 0,
    ...
    _.each(tokens, function(token) {
        var match = actionTagNameRegex.exec(token);
        if(match) {
            var tagName = match[1];
            if(tagName.slice(0, 3) !== 'end') {
                // opening tag
                levels[tagName] += 1;
            } else {
                // closing tag
                ...
                levels[tagName] -= 1;
                if(levels[tagName] < 0) { throw new errors.TemplateSyntaxError('redundant closing tag: ' + tagName); }
            }
        }
    });

    for(var tagName in levels) {
        var l = levels[tagName];
        if(l > 0) {
            throw new errors.TemplateSyntaxError('not closed tag: ' + tagName);
        }
    }
}
